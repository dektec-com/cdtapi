...
#
# CDTAPI - Compares CDTAPI's device scan with DTAPI's on a machine with a card
#
# SPDX-License-Identifier: BSD-3-Clause
#
# Builds Scripts/Compare/DtapiDeviceScanRef.cpp against DTAPI from a DekTec Linux SDK,
# runs it and CDTAPI's DtListDeviceDescs, and compares their output line by line.
#
#   Scripts/compare_device_scan.sh <LinuxSDK directory> [<CDTAPI build directory>]
#
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <LinuxSDK directory> [<CDTAPI build directory>]"
    exit 1
fi
...
"$Build/Examples/DtListDeviceDescs" > "$Work/cdtapi.txt" || true

if diff -u "$Work/dtapi.txt" "$Work/cdtapi.txt"; then
