...
static int Grow(DtVec* Vec, size_t Needed)
{
    ...
    if (DtAlloc_GrowCapacity(Vec->Capacity, Needed, Vec->ElemSize, DT_VEC_MIN_CAPACITY,
                             &NewCapacity) != 0)
    {
        return -1;
    }
    ...
    uint8_t* NewData = (uint8_t*)DtAlloc_Realloc(Vec->Data, NewCapacity * Vec->ElemSize);
    if (NewData == NULL)
        return -1;
