...
// +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+= Input status +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
//
// What is held once attached to a port of a DtPcie card, for the high-level Matrix API
// route it takes there: the port, its capabilities, and the SDI receiver driver function
// the port's ASI/SDI receiver API function names. Attaching finds that function once;
// detecting reads it as often as the caller asks.
//
...
// with the empty role; DTAPI_E_NOT_FOUND when there is none, which attaching finds out
