...
//
// A scan walks what the card has written since the last: the packets that decode are
...
// past is noted, so that delivering the bytes later decodes the same packets again
// without looking at the flags. Delivering decodes straight from the buffer into the
// caller's, keeping at most one packet's output for the next delivery.
//
