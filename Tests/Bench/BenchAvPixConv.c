...
//
// The bands run on the same DtJobRunner a channel uses, with a pool that lives for the
// whole run rather than threads started per frame, so what is measured is what a program
...
#include "Core/DtWorkerPool.h"  // The bands, over the threads a channel would use.
...
// working symbols from the piece number DtJobRunner gives it.
...
// DtJobRunner divides it into. This is the mechanism a channel uses, pool and all, so
// what is measured is what a program calling the library gets rather than a copy of it.
                           ...
                           const DtJobRunner* Work)
{
    ...
    Job.ToRaw = ToRaw;
    Job.Buf = Buf;
    DtJobRunner_Run(Work, ConvertBand4k, &Job);
}
...
static void Measure4k(const DtSdi4kConv* Conv, const DtSdiFrameLayout* Layout, int Bits,
                      ...
                      const DtJobRunner* Work, double* Ms, double* Fps, double* MCycles)
{
    uint64_t Start = OsTime_MonotonicMs();
    uint64_t Elapsed = 0;
    int Frames = 0;
...
static void Sdi4k(int Seconds, int Threads)
{
    ...
    DtWorkerPool* Pool = Threads > 1 ? DtWorkerPool_Alloc() : NULL;
    if ((Threads > 1 &&
         (Pool == NULL || DtWorkerPool_StartThreads(Pool, Threads) != DTAPI_OK)) ||
        DtJobRunner_SetPool(&Work, Pool, 0) != DTAPI_OK)
    {
